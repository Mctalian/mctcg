import { CardValidator } from "../cards/card-validator";
import { SingularityCard, isBasicEnergy, isSingularityCard } from "../cards/card.interface";
import { Singularity } from "../cards/singularity.enum";
...
export class DeckValidator {
  ...
  private validateNumberOfCards() {
    ...
    const totalPokemon = this.deck.Pokemon.reduce((acc, card) => acc + card.quantity, 0);
    const totalTrainer = this.deck.Trainer.reduce((acc, card) => acc + card.quantity, 0);
    const totalEnergy = this.deck.Energy.reduce((acc, card) => acc + card.quantity, 0);
  ...
  private validateRegulations() {
    if (this.deck.format === Format.Standard) {
      const cards = this.deck.Pokemon.concat(this.deck.Trainer).concat(this.deck.Energy);
