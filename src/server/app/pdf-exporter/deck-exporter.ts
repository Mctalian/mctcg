...
export class DeckExporter {
  ...
  private async addLogo() {
    const logoBytes = await readFile(PNG_LOGO_PATH);
    const logoImage = await this.pdfDoc.embedPng(logoBytes);
    const logoDims = logoImage.scale(1/45);
    const logoWidth = logoDims.width;
    const logoHeight = logoDims.height;
    const x = this.pageWidth - 50 - logoWidth;
    const y = this.pageHeight - 90 - logoHeight;
    ...
    this.page.drawText("Generated by", {
      x: x - 5 + (logoWidth / 2),
      y: y + logoHeight,
      rotate: {
        type: RotationTypes.Degrees,
